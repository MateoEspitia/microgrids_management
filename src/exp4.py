...
for location in locations:
    ...
    for day in days:
        ...
        for a in np.arange(0.8, 1.3, 0.1): #["low", "medium", "high"]:
            ...
            """
            if a == "low":
                generators_dict['Fict'].va_op = generators_dict['Diesel1'].va_op * 0.5
            elif a == "medium":
                generators_dict['Fict'].va_op = generators_dict['Diesel1'].va_op * 1.1
            elif a == "high":
                generators_dict['Fict'].va_op = generators_dict['Diesel1'].va_op * 1.5
            """
            generators_dict['Fict'].va_op = generators_dict['Diesel1'].va_op * a
            ...
            #Solar
            size['S'] = math.ceil((0.1*sum(i for i in demand_d))/(generators_dict['Solar1'].G_test * h_p))
            ...
            size['W'] = math.ceil((0.1*sum(i for i in demand_d))/(24*f_plan*20))
            ...
            c_diesel = 3000*s_dem
            p_cost = value(model.generation_cost)/c_diesel
            ...
            del model
        #sys.exit()
    with open(os.path.join('../results/', str(location)+'_4_1'+'.csv'), 'w', newline='') as file:
